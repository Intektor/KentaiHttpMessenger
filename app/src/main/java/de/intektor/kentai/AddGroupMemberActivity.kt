                    ...
                    override fun click(item: Contact, view: ContactViewAdapter.ViewHolder) {
                        AlertDialog.Builder(this@AddGroupMemberActivity)
                                ...
                                .setPositiveButton(R.string.add_group_member_add_alert_positive, { _, _ ->
                                    ...
                                    handleGroupModification(modification, KentaiClient.INSTANCE.dataBase)
                                    sendMessageToServer(this@AddGroupMemberActivity,
                                            PendingMessage(
                                                    ChatMessageWrapper(
                                                            ChatMessageGroupModification(
                                                                    modification, KentaiClient.INSTANCE.userUUID, System.currentTimeMillis()),
                                    ...
                                    KentaiClient.INSTANCE.dataBase.rawQuery("SELECT group_key FROM group_key_table WHERE chat_uuid = ?", arrayOf(chatInfo.chatUUID.toString())).use { query ->
                                        cursor.moveToNext()
                                        groupKey = cursor.getString(0).toAESKey() as SecretKey
                                    }

                                    KentaiClient.INSTANCE.dataBase.rawQuery("SELECT chat_uuid FROM user_to_chat_uuid WHERE user_uuid = ?", arrayOf(item.userUUID.toString())).use { query ->
                                        val chatUUID: UUID
                                        if (query.moveToNext()) {
                                        ...
                                        sendMessageToServer(this@AddGroupMemberActivity, PendingMessage(
                                                ChatMessageWrapper(ChatMessageGroupInvite(chatInfo.chatUUID, roleMap, chatInfo.chatName, groupKey!!, KentaiClient.INSTANCE.userUUID, System.currentTimeMillis()), MessageStatus.WAITING, true, System.currentTimeMillis())
                                                , chatUUID, listOf(ChatReceiver(item.userUUID, item.message_key, ChatReceiver.ReceiverType.USER))))
                                    }
                                    finish()
                                })
                                .create().show()
